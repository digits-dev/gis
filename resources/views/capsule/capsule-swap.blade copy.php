...
@endpush
@section('content')
    <div class="swap-summary-div" style="display:none;">
        <div class="swap-table1_summary styled-table-swap" >
            <table>
                <tbody>
                    <tr>
                        <td>Location</td>
        ...
        <div class="swap-table2_summary styled-table-swap" >
            <table>
                <tbody>
                    <tr>
                        <td>Location</td>
...
<script>
            ...
            data.jan_data.forEach(item => {
                const populate_row = `
                    <tr id="${item.digits_code}">
                        ...
                        <td hidden><input class="inventory-item-qty1" value="${item.qty}" ></td>
                        <td><input class="form-control capsule_qty_one" type="text" name="capsule_qty_one[]" oninput="formatNumber(this)"></td>
                    </tr>
                `;
    ...
    function displayMAchineTwoJanData(data){
        console.log(data);
        if(data.jan_data.length != 0){
            $('.styled-table-swap-two').fadeIn();
            data.jan_data.forEach(item => {
                const populate_row = `
                    <tr id="${item.digits_code}">
                        <td><input class="form-control" type="text" name="jan_no_two[]" value="${item.digits_code}" readonly></td>
                        <td hidden><input class="inventory-item-qty2" value="${item.qty}" ></td>
                        <td><input class="form-control capsule_qty_two" type="text" name="capsule_qty_two[]" oninput="formatNumber(this)"></td>
                    </tr>
                `;
    ...
    $(document).on('input','.capsule_qty_one',function(){
        console.log($(this).val());
        const row = $(this).closest('tr');
        const capsuleQtyOne = parseFloat($(this).val().replace(/,/g,'')) || 0;
        const inventoryItemQtyOne = parseFloat(row.find('.inventory-item-qty1').val()) || 0;
        if (capsuleQtyOne > inventoryItemQtyOne ) {
            $('#save-btn').attr('disabled',true);
            $('.capsule_qty_one').css('border-color','red');
        } else {
            $('#save-btn').attr('disabled',false);
            $('.capsule_qty_one').css('border-color','red');
    ...
    $(document).on('input','.capsule_qty_two',function(){
        console.log($(this).val());
        const row = $(this).closest('tr');
        const capsuleQtyTwo = parseFloat($(this).val().replace(/,/g,'')) || 0;
        const inventoryItemQtyTwo = parseFloat(row.find('.inventory-item-qty2').val()) || 0;
        if (capsuleQtyTwo > inventoryItemQtyTwo ) {
            $('#save-btn').attr('disabled',true)
        } else {
            $('#save-btn').attr('disabled',false)
        }
