...
class AdminCollectTokenController extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function saveEdit(Request $request)
	{
		...
		try {
			$validated_data = $request->validate([
				...
				'header_collected_qty' => 'required',
				'header_received_qty' => 'nullable',
				'header_variace' => 'required',
				//lines
				'collect_token_lines_id' => 'required|array',
				'collect_token_lines_no_of_token' => 'array',
				'collect_token_lines_collected_qty' => 'array',
				'collect_token_lines_variance' => 'array',
				'collect_token_lines_variance_type' => 'array',
				'collect_token_lines_projected_capsule_sales' => 'array',
				'collect_token_lines_current_capsule_inventory' => 'array',
				'collect_token_lines_actual_capsule_inventory' => 'array',
				'collect_token_lines_actual_capsule_sales' => 'array',
			]);
		...
		// collect token lines to mapping each array 
		$Validated_lines = array_map(
			function (
				$collect_token_lines_id,
				$lines_no_of_token,
				$lines_collected_qty,
				$lines_variance,
				$lines_variance_type,
				$lines_projected_capsule_sales,
				$lines_current_capsule_inventory,
				$lines_actual_capsule_inventory,
				$lines_actual_capsule_sales
			) {
				return [
					'collect_token_lines_id' => $collect_token_lines_id,
					'collect_token_lines_no_of_token' => $lines_no_of_token,
					'collect_token_lines_collected_qty' => $lines_collected_qty,
					'collect_token_lines_variance' => $lines_variance,
					'collect_token_lines_variance_type' => $lines_variance_type,
					'collect_token_lines_projected_capsule_sales' => $lines_projected_capsule_sales,
					'collect_token_lines_current_capsule_inventory' => $lines_current_capsule_inventory,
					'collect_token_lines_actual_capsule_inventory' => $lines_actual_capsule_inventory,
					'collect_token_lines_actual_capsule_sales' => $lines_actual_capsule_sales,
				];
			},
			$validated_data['collect_token_lines_id'],
			$validated_data['collect_token_lines_no_of_token'],
			$validated_data['collect_token_lines_collected_qty'],
			$validated_data['collect_token_lines_variance'],
			$validated_data['collect_token_lines_variance_type'],
			$validated_data['collect_token_lines_projected_capsule_sales'],
			$validated_data['collect_token_lines_current_capsule_inventory'],
			$validated_data['collect_token_lines_actual_capsule_inventory'],
			$validated_data['collect_token_lines_actual_capsule_sales']
		);
		...
		foreach ($Validated_lines as $per_item) {
			$updated = $collectTokenHeader->lines()->where('id', $per_item['collect_token_lines_id'])->update([
				...
				'variance_type' => $per_item['collect_token_lines_variance_type'],
				'projected_capsule_sales' => $per_item['collect_token_lines_projected_capsule_sales'],
				'actual_capsule_sales' => $per_item['collect_token_lines_actual_capsule_sales'],
				'current_capsule_inventory' => $per_item['collect_token_lines_current_capsule_inventory'],
				'actual_capsule_inventory' => $per_item['collect_token_lines_actual_capsule_inventory'],
				'updated_at' => now(),
			]);
	...
	public function postCollectTokenApproval(Request $request)
	{
		...
		try {
			$collectTokenHeader = CollectRrTokens::findOrFail($request['collect_token_id']);
			...
			if ($request->action_type == 'approve') {
				...
				$validatedData = $request->validate([
					'ref_number' => 'required',
					'header_location_id' => 'required',
					'total_collected_token' => 'required|numeric|min:1',
					'collect_token_id' => 'required',
					'collect_token_lines_ids' => 'required|array',
					'location_id' => 'required|array',
					'jan_code' => 'required|array',
					'item_desc' => 'required|array',
					'item_code' => 'required|array',
					'gasha_machines_id' => 'required|array',
					'no_of_tokens' => 'required|array',
					'collected_qty' => 'required|array',
					'variance' => 'required|array',
					'variance_type' => 'required|array',
					'projected_capsule_sales' => 'required|array',
					'actual_capsule_sales' => 'required|array',
					'current_capsule_inventory' => 'required|array',
					'actual_capsule_inventory' => 'required|array',
					'line_created_at' => 'required|array',
					'line_updated_at' => 'required|array',
					'inventory_capsule_lines_id' => 'required|array',
				]);

				// Process Capsule Sales
				foreach ($validatedData['jan_code'] as $index => $jan_code) {
					$capsuleSalesData = [
						'reference_number' => $validatedData['ref_number'],
						'item_code' => $jan_code,
						'gasha_machines_id' => $validatedData['gasha_machines_id'][$index],
						'locations_id' => $validatedData['location_id'][$index],
						'qty' => $validatedData['actual_capsule_sales'][$index],
						'sales_type_id' => SalesType::COLLECTTOKEN,
						'created_by' => CRUDBooster::myId(),
						'created_at' => now()
					];

					if ($capsuleSalesData['qty'] > 0) {
						CapsuleSales::firstOrCreate($capsuleSalesData);
						HistoryCapsule::firstOrCreate([
							'reference_number' => $validatedData['ref_number'],
							'item_code' => $capsuleSalesData['item_code'],
							'capsule_action_types_id' => CapsuleActionType::COLLECTTOKEN,
							'locations_id' => $capsuleSalesData['locations_id'],
							'gasha_machines_id' => $capsuleSalesData['gasha_machines_id'],
							'qty' => -abs($capsuleSalesData['qty']),
							'to_machines_id' => $capsuleSalesData['gasha_machines_id'],
				...
				foreach ($validatedData['inventory_capsule_lines_id'] as $index => $lineId) {
					$inventoryLine = InventoryCapsuleLine::where('id', $lineId)->lockForUpdate()->first();

					if ($inventoryLine) {
						$deducted_qty = max(0, $inventoryLine->qty - $validatedData['actual_capsule_sales'][$index]);

						InventoryCapsuleLine::where('id', $lineId)->update([
							'qty' => $deducted_qty,
				...
				$tokenInventory = TokenInventory::where('locations_id', $validatedData['header_location_id'])
					->lockForUpdate()
					->first();

				if ($tokenInventory) {
					$tokenInventory->increment('qty', $validatedData['total_collected_token'], [
						'updated_by' => CRUDBooster::myId(),
						'updated_at' => now()
					]);
				}
