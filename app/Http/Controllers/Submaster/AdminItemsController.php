<?php namespace App\Http\Controllers\Submaster;
	...
	class AdminItemsController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form = [];
			$this->form[] = ['label'=>'Digits Code','name'=>'digits_code','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-5'];
			$this->form[] = ['label'=>'Item Description','name'=>'item_description','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-5'];
			$this->form[] = ['label'=>'No Of Tokens','name'=>'no_of_tokens','type'=>'number','validation'=>'required|integer|min:0','width'=>'col-sm-5'];
	    ...
	    */
        ...
        public function getItemsCreatedAPI()
        {
            ...
            if($userAgent == '' || is_null($userAgent)){
                $userAgent = config('api.user_agent');
            }
        ...
        public function getItemsUpdatedAPI()
        {
            ...
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL,config('api.dimfs_api_updated_link'));
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
            curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 1);
            curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
            curl_setopt($ch, CURLOPT_POST, FALSE);
